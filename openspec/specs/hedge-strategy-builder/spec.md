...
The system SHALL display a Lightweight Charts line chart showing the strategy's net value over time, calculated from historical OHLC data via mzdata API.

#### Scenario: Chart rendered for completed strategy
- **WHEN** user has configured at least one leg with valid strike and expiry
- **THEN** system fetches OHLC data for each leg via `getOptionHistoricalOhlc()` and renders a line chart showing net strategy value

#### Scenario: Debit strategy shown as negative
- **WHEN** strategy has a net debit (costs money)
- **THEN** chart shows values below zero (negative y-axis)

#### Scenario: Credit strategy shown as positive
- **WHEN** strategy has a net credit (receives money)
